...
cursor = con.cursor()
cursor.execute("select * from profilemaster limit 0,20")
rows = cursor.fetchall()
i=0 
for profilemaster in rows: 
    for j in range(len(profilemaster)):
        e = Entry(window,width=10,fg="white",bg='#0B446D',font=('arial',14))
        e.grid(row=i, column=j) 
        e.insert(END, profilemaster[j])
    i=i+1
con.close()
