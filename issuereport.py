...
window=Tk()
window.title('Loan Issue Report')
...
cursor = con.cursor()
cursor.execute("select * from loanissuemaster limit 0,20")
rows = cursor.fetchall()
i=0 
for loanissuemaster in rows: 
    for j in range(len(loanissuemaster)):
        e = Entry(window,width=10,fg="white",bg='#0B446D',font=('arial',14))
        e.grid(row=i, column=j) 
        e.insert(END, loanissuemaster[j])
    i=i+1
con.close()
