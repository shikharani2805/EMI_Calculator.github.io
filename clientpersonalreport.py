...
window=Tk()
window.title('Client personal Report')
...
cursor = con.cursor()
cursor.execute("select * from clientpersonal limit 0,20")
rows = cursor.fetchall()
i=0 
for clientpersonal in rows: 
    for j in range(len(clientpersonal)):
        e = Entry(window,width=10,fg="white",bg='#0B446D',font=('arial',14))
        e.grid(row=i, column=j) 
        e.insert(END, clientpersonal[j])
    i=i+1
con.close()
