...
cursor = con.cursor()
cursor.execute("select * from planmaster limit 0,20")
rows = cursor.fetchall()
i=0 
for planmaster in rows: 
    for j in range(len(planmaster)):
        e = Entry(window,width=10,fg="white",bg='#0B446D',font=('arial',14))
        e.grid(row=i, column=j) 
        e.insert(END, planmaster[j])
    i=i+1
con.close()
